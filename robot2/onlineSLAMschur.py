    ...
    for k in range(len(data)):
        ...
        for i in range(len(measurement)):
            ...
            for b in range(2):
                ...
                Xi.value[n+b][0]      += -measurement[i][1+b] / measurement_noise
                Xi.value[m+b][0]      +=  measurement[i][1+b] / measurement_noise
        #EXPAND OMEGA TO ACOMMODATE FOR MOTION
        positions = list(range(dim+2))
        positions.pop(2)
        positions.pop(2)
        Omega=Omega.expand(dim+2,dim+2,positions,positions)
        Xi=Xi.expand(dim+2,1,positions,[0])
        # update the information maxtrix/vector based on the robot motion
        for b in range(4):
            Omega.value[n+b][n+b] +=  1.0 / motion_noise
        for b in range(2):
        ...
        B=Omega.take([0,1])
        Binv=B.inverse()
        A=Omega.take([0,1],positions)
        At=Omega.take(positions,[0,1])
        Omegaprime=Omega.take(positions)
        Omega=Omegaprime- At*Binv*A
        C=Xi.take([0,1],[0])
        Xiprime=Xi.take(positions,[0])
        Xi=Xiprime-At*Binv*C
    mu=Omega.inverse()*Xi
