# -*- coding: utf-8 -*-
"""
Created on Mon Mar  2 11:20:48 2020
...
max_steering_angle = pi / 4.0  # You do not need to use this value,
# but keep in mind the limitations of a real car.
...
class robot:
    ...
    def move(self, motion):  # Do not change the name of this function
        ...
        # ADD CODE HERE
        robot_new = robot(self.length)
        robot_new.bearing_noise = self.bearing_noise
        robot_new.steering_noise = self.steering_noise
        robot_new.distance_noise=self.distance_noise
        ...
        if (abs(beta) < 0.001):
            xnew = self.x + d*cos(self.orientation)
            ynew = self.y + d*sin(self.orientation)
        else:
            R = d/beta
            cx = self.x - sin(self.orientation) * R
            cy = self.y + cos(self.orientation) * R
            xnew = cx + sin(beta + self.orientation) * R
            ynew = cy - cos((beta + self.orientation)) * R
        thetanew = (beta + self.orientation) % (2 * pi)
        robot_new.set(xnew, ynew, thetanew)
        return robot_new  # make sure your move function returns an instance
    ...
    def sense(self, noise=1):  # do not change the name of this function
        ...
        for lm in landmarks:
            ...
            Z.append((atan2(dy, dx)-self.orientation+2*pi
                      +random.gauss(0.0, self.bearing_noise))%(2*pi))
